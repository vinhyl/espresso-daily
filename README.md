...
- **维护**：`python scripts/new_knowledge.py "9 bar 水压"` 生成主题模板；人工补充/校订综合内容与来源即可。**长期运营方法论**（收集渠道、入库筛选、可靠性评判、巡检节奏、候选池）见 **`knowledge/README.md`**——执行任何知识库维护任务前应先读它。当前库约 23 篇 / 18K 字符，`max_chars = 24000`；保持 `mode = "all"` 最全面，当库总量达到预算约 80% 或注入成本明显上升时，评估切 `mode = "recall"`（按标签/概念高召回）以控 token。
