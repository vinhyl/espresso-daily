...
def _llm_prescreen(cfg: dict, item: dict, hint: str) -> dict | None:
    ...
    prompt = (
        ...
        "2. 意式生态/行业相关：意式设备品牌的新品发布、商用设备动态、**全球级**咖啡师教育/赛事/"
        "行业活动（如 World Coffee Championship 系列、WBC/WBrC/世界杯拉花赛、SCA Expo 等国际性"
        "赛事与展会），或研究对象是意式饮品/器具的研究（如 espresso、moka pot、意式机萃取）——"
        "即便标题不含 espresso 字样也放行。注意：**地区性/小型活动不在此列**，见形态检查第 10 条。\n"
        ...
        "9. 广告/促销/软文/抽奖/优惠码。\n"
        "10. 地区性/小型海外行业活动：在特定城市举办、受众为当地读者的教育/展会/赛事"
        "（如某城市教育盛会、本地快闪活动、区域性比赛、单一城市咖啡节）——本站以国内读者为主，"
        "此类活动对国内读者无实操价值；**仅全球级活动（World Coffee Championship 系列、"
        "WBC/WBrC/世界杯拉花赛、SCA Expo 等国际性赛事与展会）放行**。\n\n"
